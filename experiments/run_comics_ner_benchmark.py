"""
Comics Character NER Benchmark Runner

Runs NER benchmark on the Comics Character dataset with 66 samples
covering comic book characters, creators, and related entities.
...
from src.extractors.gliner1 import GLiNER1EntityExtractor
...
from src.models import EvaluationMetrics, NERBenchmarkResult
...
def save_summary(results_summary: list, output_path: Path) -> None:
    """Save summary to text file."""
    with open(output_path, "w") as f:
        f.write("=" * 60 + "\n")
        f.write("SUMMARY - Comics Character NER\n")
        f.write("=" * 60 + "\n")
...
def main():
    # Configuration
    benchmark_path = "datasets/ner/comics_ner_benchmark.json"
    output_dir = Path("result")
    output_dir.mkdir(exist_ok=True)
    
    # Load benchmark data
    print("=" * 60)
    print("Comics Character NER Benchmark")
    print("=" * 60)
    ...
    gliner2_extractor = GLiNER2EntityExtractor(model_id="fastino/gliner2-base-v1")
    gliner2_runner = NERBenchmarkRunner(data_loader, gliner2_extractor)
    gliner2_result = gliner2_runner.run()
    gliner2_metrics = evaluator.evaluate(gliner2_result)
    
    print_metrics(gliner2_metrics)
    print(f"  Avg time/sample: {gliner2_result.average_time_per_sample:.4f}s")
    save_results(gliner2_result, gliner2_metrics, output_dir / "gliner2_comics_ner_results.json")
    results_summary.append(("GLiNER2", gliner2_metrics, gliner2_result.average_time_per_sample))
    print()
    ...
    save_results(gliner1_result, gliner1_metrics, output_dir / "gliner1_comics_ner_results.json")
    ...
    save_results(neo4j_result, neo4j_metrics, output_dir / "neo4j_comics_ner_results.json")
    ...
    save_results(lang_result, lang_metrics, output_dir / "lang_comics_ner_results.json")
    ...
    # Save summary
    save_summary(results_summary, output_dir / "comics_ner_eval_summary.txt")
